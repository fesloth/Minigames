...
function chooseRole(role) {
  playerRole = role;
  botRole = role === "X" ? "O" : "X";
  document.getElementById("status").textContent = `Player: ${playerRole} | Bot: ${botRole}`;
}
...
function playGame() {
  ...
  window.makeMove = function (index) {
    if (!isGameOver() && board[index] === " ") {
      ...
      displayBoard();
      currentPlayer = currentPlayer === playerRole ? botRole : playerRole; // Switch the current player

      if (isGameOver()) {
        if (isWinner(playerRole)) {
          // Use playerRole instead of player
          document.getElementById("status").innerText = "Selamat! Anda menang!";
        } else if (isWinner(botRole)) {
          // Use botRole instead of bot
